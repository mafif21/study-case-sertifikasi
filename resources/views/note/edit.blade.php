            ...
            <div class="col-md-5 border py-3 px-4 rounded">
                <form action="{{ route('note.update', $note->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" id="category"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected($note->category_id == $category->id)>{{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="content" rows="3" name="content" style="resize:none">{{ $note->content }}</textarea>
                    </div>
